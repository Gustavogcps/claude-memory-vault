...
This is exactly the threat model the tool exists for ("someone changed the mirror"), and a directory symlink at the mirror root is precisely such a change. The fix is one guard — **4b**: refuse outright if `Claude/Memory` is a symlink or not a directory. It's now covered by a regression test ([test 4 in `tests/run-tests.sh`](../tests/run-tests.sh)) that asserts outside data survives.
